...
Act exaclty as CORE::open, but also work with remote HTTP files.
Falls back to CORE::open when the path looks like a local file.
...
functions are exported
   ...
   use File::HTTP qw(:open);
   # same as:
   use File::HTTP qw(open stat _s _e);
